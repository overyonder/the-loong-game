...
def main():
    ...
    parts.append(text((first + fourth) / 2, CARD_Y - 58, "v4 plays exactly like v1", 17, SIGNAL, weight="bold"))
    ...
    for index, (move, cx) in enumerate(zip(MOVES, centres)):
        ...
        parts += [
            ...
            text(cx, CARD_Y + 94, move, 16, PAPER, weight="bold"),
            text(cx, CARD_Y + CARD_HEIGHT + 34, f"v{index + 1}", 24, SIGNAL, weight="bold"),
        ]
        if index:
            ...
            parts.append(text(gap_left + GAP / 2, CARD_Y + CARD_HEIGHT / 2 - 8, "beats", 13, MUTED))
    ...
    parts.append(text(WIDTH / 2, label_y + 24, "Each version beat the one before it, so the numbers look like steady progress.", 16, MUTED, style="italic"))
    parts.append("</svg>")
